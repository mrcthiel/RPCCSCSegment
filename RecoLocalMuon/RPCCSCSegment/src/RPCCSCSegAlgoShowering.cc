/**
 * \file RPCCSCSegAlgoShowering.cc
 *
 ...
 *
 */
RPCCSCSegAlgoShowering::RPCCSCSegAlgoShowering(const edm::ParameterSet& ps) : sfit_(nullptr) {
//  debug                  = ps.getUntrackedParameter<bool>("RPCCSCSegmentDebug");
 ...
 *
 */
RPCCSCSegAlgoShowering::~RPCCSCSegAlgoShowering(){
 ...
 *
 */
RPCCSCSegment RPCCSCSegAlgoShowering::showerSeg( const CSCChamber* aChamber, const ChamberHitContainer& rechits ) {
 ...
 */
bool RPCCSCSegAlgoShowering::isHitNearSegment( const CSCRecHit2D* hit ) const {
 ...
 *
 */
bool RPCCSCSegAlgoShowering::addHit(const CSCRecHit2D* aHit, int layer) {
 ...
 *
 */ 
void RPCCSCSegAlgoShowering::compareProtoSegment(const CSCRecHit2D* h, int layer) {
  ...
  if (ok) {
    RPCCSCSegFit* newfit = new RPCCSCSegFit(theChamber, protoSegment);
    newfit->fit();
...
void RPCCSCSegAlgoShowering::pruneFromResidual(void){
...
void RPCCSCSegAlgoShowering::updateParameters(void) {
  // Create fit for the hits in the protosegment & run it
  delete sfit_;
  sfit_ = new RPCCSCSegFit( theChamber, protoSegment );
  sfit_->fit();
}
