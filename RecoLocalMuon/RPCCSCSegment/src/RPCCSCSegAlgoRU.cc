...
std::vector<RPCCSCSegment> RPCCSCSegAlgoRU::buildSegments(const CSCChamber* aChamber, const ChamberHitContainer& urechits) const {
  ...
  if (rechits.size()>150){
    return std::vector<RPCCSCSegment>();
  }
  int iadd = 0;
  ...
  // This is going to point to fits to hits, and its content will be used to create a RPCCSCSegment
  AlgoState aState;
  ...
  std::vector<RPCCSCSegment> segments;
  ...
  for (int ipass = 0; ipass < npass; ++ipass) {
    ...
    std::vector<RPCCSCSegment>::iterator it =segments.begin();
...
bool RPCCSCSegAlgoRU::areHitsCloseInR(const AlgoState& aState, const CSCRecHit2D* h1, const CSCRecHit2D* h2) const {
...
bool RPCCSCSegAlgoRU::areHitsCloseInGlobalPhi(const AlgoState& aState, const CSCRecHit2D* h1, const CSCRecHit2D* h2) const {
...
bool RPCCSCSegAlgoRU::isHitNearSegment(const AlgoState& aState, const CSCRecHit2D* h) const {
  // Is hit near segment?
...
float RPCCSCSegAlgoRU::phiAtZ(const AlgoState& aState, float z) const {
...
bool RPCCSCSegAlgoRU::isSegmentGood(const AlgoState& aState,  const ChamberHitContainer& rechitsInChamber) const {
...
void RPCCSCSegAlgoRU::flagHitsAsUsed(const AlgoState& aState, const ChamberHitContainer& rechitsInChamber,
...
bool RPCCSCSegAlgoRU::addHit(AlgoState& aState, const CSCRecHit2D* aHit, int layer) const {
...
void RPCCSCSegAlgoRU::updateParameters(AlgoState& aState) const {
  // Delete input RPCCSCSegFit, create a new one and make the fit
  // delete sfit;
  aState.sfit.reset(new RPCCSCSegFit( aState.aChamber, aState.proto_segment ));
  aState.sfit->fit();
}

float RPCCSCSegAlgoRU::fit_r_phi(const AlgoState& aState, const SVector6& points, int layer) const{
...
void RPCCSCSegAlgoRU::baseline(AlgoState& aState, int n_seg_min) const {
...
float RPCCSCSegAlgoRU::fitX(const AlgoState& aState, SVector6 points, SVector6 errors, int ir, int ir2, float &chi2_str) const {
  float S = 0;
  float Sx = 0;
  float Sy = 0;
  float Sxx = 0;
...
bool RPCCSCSegAlgoRU::hasHitOnLayer(const AlgoState& aState, int layer) const {
...
bool RPCCSCSegAlgoRU::replaceHit(AlgoState& aState, const CSCRecHit2D* h, int layer) const {
...
void RPCCSCSegAlgoRU::compareProtoSegment(AlgoState& aState, const CSCRecHit2D* h, int layer) const {
  // Copy the input RPCCSCSegFit
  std::unique_ptr<RPCCSCSegFit> oldfit;
  oldfit.reset(new RPCCSCSegFit( aState.aChamber, aState.proto_segment ));
  oldfit->fit();
...
void RPCCSCSegAlgoRU::increaseProtoSegment(AlgoState& aState, const CSCRecHit2D* h, int layer, int chi2_factor) const {
  // Creates a new fit
  std::unique_ptr<RPCCSCSegFit> oldfit;
  ChamberHitContainer oldproto = aState.proto_segment;
  oldfit.reset(new RPCCSCSegFit( aState.aChamber, aState.proto_segment ));
