...
void CSCCondSegFit::fit( bool condpass1, bool condpass2 ) {
  ...
  // See base class RPCCSCSegFit::fit for detailed explanation of algorithm.
  ...
  if(condpass1 && !condpass2){
    ...
    if(lex_.size()!=hits_.size()){
      LogTrace("RPCCSCSegment|CSC") << "[RPCCSCSegFit::fit] lex_.size()!=hits_.size() ALARM! Please inform CSC DPG " << std::endl;
    }
  }
  ...
  for (ih = hits_.begin(); ih != hits_.end(); ++ih) {
    ...
    bool ok = IC.Invert();
    if ( !ok ) {
      LogTrace("RPCCSCSegment|CSC") << "[RPCCSCSegFit::fit] Failed to invert covariance matrix: \n" << IC;      
  ...
  bool ok = M.Invert();
  if (!ok ){
    LogTrace("RPCCSCSegment|CSC") << "[RPCCSCSegFit::fit] Failed to invert matrix: \n" << M;
    //    return ok; //@@ SHOULD PASS THIS BACK TO CALLER?
  }
  else {
    p = M * B;
...
void CSCCondSegFit::setChi2( bool condpass1, bool condpass2 ) {
  ...
  for (ih = hits_.begin(); ih != hits_.end(); ++ih) {
    ...
    bool ok = IC.Invert();
    if (!ok ){
      LogTrace("RPCCSCSegment|CSC") << "[RPCCSCSegFit::setChi2] Failed to invert covariance matrix: \n" << IC;
      //      return ok;
    }
