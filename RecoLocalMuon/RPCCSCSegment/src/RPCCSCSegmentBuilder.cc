...
RPCCSCSegmentBuilder::RPCCSCSegmentBuilder(const edm::ParameterSet& ps) : geom_(nullptr) {
    ...
    LogDebug("RPCCSCSegment|CSC")<< "RPCCSCSegmentBuilder algorithm name: " << algoName;
    ...
    LogDebug("RPCCSCSegment|CSC")<< "No. of chamber types to handle: " << chType.size();
    ...
    for (size_t j=0; j<chType.size(); ++j) {
        algoMap.emplace(chType[j], RPCCSCSegmentBuilderPluginFactory::get()->
