#ifndef RPCCSCSegment_RPCCSCSegAlgoRU_h
#define RPCCSCSegment_RPCCSCSegAlgoRU_h

/**
 * \class RPCCSCSegAlgoRU
 *
 ...
 *
 * A RPCCSCSegment is a RecSegment4D, and is built from
 * CSCRecHit2D objects, each of which is a RecHit2DLocalPos. <BR>
 *
 * This class is used by the RPCCSCSegmentAlgorithm. <BR>
 ...
 * used via the RPCCSCSegmentBuilder. <BR>
 *
...
class RPCCSCSegAlgoRU : public RPCCSCSegmentAlgorithm {
...
public:
    ...
    /// Constructor
    explicit RPCCSCSegAlgoRU(const edm::ParameterSet& ps);
    /// Destructor
    ~RPCCSCSegAlgoRU() override {};
...
private:
    struct AlgoState {
      ...
      int strip_iadd = 0;
      int chi2D_iadd = 0;
      std::unique_ptr<RPCCSCSegFit> sfit = nullptr;
    ...
    // might like RPCCSCSegmentizer-specific behaviour?
