...
void RPCCSCSegFit::fit(void) {
  if ( fitdone() ) return; // don't redo fit unnecessarily
  short n = nhits();
  switch ( n ) {
  case 1:
    edm::LogVerbatim("RPCCSCSegFit") << "[RPCCSCSegFit::fit] - cannot fit just 1 hit!!";
    break;
  case 2:
    fit2();
    break;
  ...
  case 6:
    ...
    break;
  default:
    edm::LogVerbatim("RPCCSCSegFit") << "[RPCCSCSegFit::fit] - cannot fit more than 6 hits!!";
  }  
}

void RPCCSCSegFit::fit2(void) {
  ...
  if (il1 == il2) {
    edm::LogVerbatim("RPCCSCSegFit") << "[RPCCSCSegFit:fit]2 - 2 hits on same layer!!";
    return;
  }
...
void RPCCSCSegFit::fitlsq(void) {
  
  // Linear least-squares fit to up to 6 CSC rechits, one per layer in a CSC.
  // Comments adapted from  mine in original  RPCCSCSegAlgoSK algorithm.
  ...
  for (ih = hits_.begin(); ih != hits_.end(); ++ih) {
    ...
    bool ok = IC.Invert();
    if ( !ok ) {
      edm::LogVerbatim("RPCCSCSegment|RPCCSCSegFit") << "[RPCCSCSegFit::fit] Failed to invert covariance matrix: \n" << IC;      
  ...
  bool ok = M.Invert();
  if (!ok ){
    edm::LogVerbatim("RPCCSCSegment|RPCCSCSegFit") << "[RPCCSCSegFit::fit] Failed to invert matrix: \n" << M;
    //    return ok; //@@ SHOULD PASS THIS BACK TO CALLER?
  }
  else {
    p = M * B;
  ...
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::fit] p = " 
  //        << p(0) << ", " << p(1) << ", " << p(2) << ", " << p(3);
...
void RPCCSCSegFit::setChi2(void) {
  ...
  for (ih = hits_.begin(); ih != hits_.end(); ++ih) {
    ...
    //    LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::setChi2] u, v, z = " << u << ", " << v << ", " << z;
    ...
    //    LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::setChi2] IC before = \n" << IC;

    // Invert covariance matrix
    bool ok = IC.Invert();
    if (!ok ){
      edm::LogVerbatim("RPCCSCSegment|RPCCSCSegFit") << "[RPCCSCSegFit::setChi2] Failed to invert covariance matrix: \n" << IC;
      //      return ok;
    }
    //    LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::setChi2] IC after = \n" << IC;
  ...
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::setChi2] chi2 = " << chi2_ << "/" << ndof_ << " dof";
...
RPCCSCSegFit::SMatrixSym12 RPCCSCSegFit::weightMatrix() {
...
// Note scaleXError allows rescaling the x error if necessary
  ...
  if ( !ok ) {
    edm::LogVerbatim("RPCCSCSegment|RPCCSCSegFit") << "[RPCCSCSegFit::weightMatrix] Failed to invert matrix: \n" << matrix;      
    //    return ok; //@@ SHOULD PASS THIS BACK TO CALLER?
  }
  return matrix;
}
...
RPCCSCSegFit::SMatrix12by4 RPCCSCSegFit::derivativeMatrix() {
...
void RPCCSCSegFit::setOutFromIP() {
...
AlgebraicSymMatrix RPCCSCSegFit::covarianceMatrix() {
  
  SMatrixSym12 weights = weightMatrix();
  SMatrix12by4 A = derivativeMatrix();
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::covarianceMatrix] weights matrix W: \n" << weights;      
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::covarianceMatrix] derivatives matrix A: \n" << A;      
  ...
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::covarianceMatrix] (AT W A): \n" << result;      
  ok = result.Invert(); // inverts in place
  if ( !ok ) {
    edm::LogVerbatim("RPCCSCSegment|RPCCSCSegFit") << "[RPCCSCSegFit::calculateError] Failed to invert matrix: \n" << result;      
    //    return ok;  //@@ SHOULD PASS THIS BACK TO CALLER?
  }
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::covarianceMatrix] (AT W A)^-1: \n" << result;      
  
  // reorder components to match TrackingRecHit interface (RPCCSCSegment isa TrackingRecHit)
...
AlgebraicSymMatrix RPCCSCSegFit::flipErrors( const SMatrixSym4& a ) { 
    
  // The RPCCSCSegment needs the error matrix re-arranged to match
  // parameters in order (uz, vz, u0, v0) 
  // where uz, vz = slopes, u0, v0 = intercepts
    
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::flipErrors] input: \n" << a;      
  ...
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::flipErrors] after copy:";
  //  LogTrace("RPCCSCSegFit") << "(" << hold(1,1) << "  " << hold(1,2) << "  " << hold(1,3) << "  " << hold(1,4);
  //  LogTrace("RPCCSCSegFit") << " " << hold(2,1) << "  " << hold(2,2) << "  " << hold(2,3) << "  " << hold(2,4);
  //  LogTrace("RPCCSCSegFit") << " " << hold(3,1) << "  " << hold(3,2) << "  " << hold(3,3) << "  " << hold(3,4);
  //  LogTrace("RPCCSCSegFit") << " " << hold(4,1) << "  " << hold(4,2) << "  " << hold(4,3) << "  " << hold(4,4) << ")";
  ...
  //  LogTrace("RPCCSCSegFit") << "[RPCCSCSegFit::flipErrors] after flip:";
  //  LogTrace("RPCCSCSegFit") << "(" << hold(1,1) << "  " << hold(1,2) << "  " << hold(1,3) << "  " << hold(1,4);
  //  LogTrace("RPCCSCSegFit") << " " << hold(2,1) << "  " << hold(2,2) << "  " << hold(2,3) << "  " << hold(2,4);
  //  LogTrace("RPCCSCSegFit") << " " << hold(3,1) << "  " << hold(3,2) << "  " << hold(3,3) << "  " << hold(3,4);
  //  LogTrace("RPCCSCSegFit") << " " << hold(4,1) << "  " << hold(4,2) << "  " << hold(4,3) << "  " << hold(4,4) << ")";
...
float RPCCSCSegFit::xfit( float z ) const {
  //@@ ADD THIS TO EACH ACCESSOR OF FIT RESULTS?
  //  if ( !fitdone() ) fit();
  return intercept_.x() + uslope_ * z;
}

float RPCCSCSegFit::yfit( float z ) const {
  return intercept_.y() + vslope_ * z;
}

float RPCCSCSegFit::xdev( float x, float z ) const {
  return intercept_.x() + uslope_ * z - x;
}

float RPCCSCSegFit::ydev( float y, float z ) const {
  return intercept_.y() + vslope_ * z - y;
}

float RPCCSCSegFit::Rdev(float x, float y, float z) const {
